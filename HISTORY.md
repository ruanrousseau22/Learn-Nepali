...
- `og-image.png` + `og-card.svg` — the 1200×630 social share card (referenced by
  `og:image` / `twitter:card summary_large_image` in the head, in `index.html`
  AND in `gen_landing.py`, so all 12 pages share one file) and its editable SVG
  source. **Rebuilt July 2026** (Ruan): the scenery is no longer a hand-drawn
  approximation — it is the app's OWN default (Nepali) hero art, and the tagline
  dropped ", free" to read "Learn Asian languages".
  **Regeneration recipe** (this Mac has no Node and no SVG rasteriser —
  rsvg-convert/inkscape/ImageMagick/cairosvg are all absent; qlmanage mangles
  non-square SVGs). Do it all in the browser preview, which has the real fonts:
  1. Open the app in Nepali, LIGHT theme, `show('home')`.
  2. Serialise `.hero-mtns`: clone it, walk every element, copy computed
     `fill`/`stroke`/`opacity` onto presentation attributes, drop `class`/`style`,
     and DELETE any element whose computed display/visibility/opacity hides it
     (this is what removes the night-only `.hero-stars`). The art has no `<defs>`
     or gradients, so flat literals are a complete capture.
  3. Inline Fraunces: `fetch` the Google Fonts css2 API for
     `Fraunces:ital,opsz,wght@0,9..144,600;1,9..144,600`, take the two blocks whose
     unicode-range starts `U+0000-00FF`, fetch each woff2 (gstatic sends CORS `*`),
     base64 them, and emit `@font-face` rules with `data:` URLs inside the SVG's
     `<style>`. A data-URI font inside a data-URI SVG renders on canvas; a plain
     Google Fonts `url()` does NOT (the img has no network).
  4. Compose 1200×630: sky `#CFE4F7`→`#EFF4FB` (the app's own hero gradient),
     art at `translate(0,310)` at natural 1:1, then the tree mark + wordmark.
     The lockup is centred on x=600: treeW 113 + gap 42 + "Bhasaly" 368 = 523,
     tree at y=155, baseline y=278, tagline y=352.
  5. Rasterise: `new Image()` from `data:image/svg+xml;charset=utf-8,`+encodeURIComponent,
     draw to a 1200×630 canvas, `toDataURL('image/png')`. NOT tainted — the SVG
     has no external references once the fonts are inlined.
  6. Get the bytes out: `javascript_tool` returns >~50k chars to a tool-result
     FILE, so slice the base64 into 50k chunks, `jq -r '.[0].text'` each saved
     file, `json.loads` to unescape, concatenate, `base64.b64decode`. Verify:
     length matches, IHDR says 1200×630, and every chunk CRC validates.
  Do NOT commit the font-embedded SVG as `og-card.svg` (it is ~114KB of base64);
  the committed source references Fraunces by name and is ~10KB.
  **Chat apps and Facebook/Twitter cache link previews by URL for days**, so the
  `og:image` URL carries a `?v=N` — bump N whenever the image changes, in BOTH
  `index.html` and `gen_landing.py`, then rerun `python3 gen_landing.py`.
