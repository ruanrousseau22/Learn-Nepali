...
- `learn-<slug>.html` (11) + `<slug>-alphabet.html` (9, non-Latin only) +
  ...
  **`sitemap.xml` `lastmod` is content-addressed via `.lastmod.json`**: a date
  ...
  The app footer (`.foot-langs`) links all 12.
- `sitemap.xml` — GENERATED by `gen_landing.py`; `robots.txt` — allow-all.
- `indexnow.py` + `<key>.txt` — IndexNow ping for Bing / DuckDuckGo / Yandex
  (**Google does not participate**). **Run it AFTER a deploy, never before**:
  IndexNow fetches the key file AND the submitted URLs from the live site, so
  an early ping burns the submission on stale content. Default mode submits
  only URLs whose sitemap `lastmod` is today — which is exactly what the
  content-addressed `.lastmod.json` just computed. **The key file is the
...
surface** (hero h1 native, English the quiet second line). FIVE stories
since the Aug 2026 library restructure (flives + fmeet were REPLACED):
`fgospel` (solo, The Gospel in one page), `fstory` (God's Story, 10
sections), `fjesus` (Jesus, 10 sections + the Who-is-Jesus overview),
`facts` (Jesus' Followers / Acts, 10 sections), `ffollow` (Following
Jesus Yourself, 4 sections) — 255 spoken strings per pack, EN/refs
byte-identical across all 12 packs EXCEPT the localized country strings
(each pack names its own country; xh honestly says MANY follow Jesus in
South Africa where the others say some). No XP/exercises;
...
- Content: `faith/<code>.js`, `registerFaith({code, digits?, ui, stories})`;
  ...
  field name always holds the NATIVE string. All 12 languages are fully
  translated (255 clips each). English `[2]` and refs `[3]` stay
...
- **Xhosa is the exception to everything edge-tts below**: no edge-tts voice
  exists — `generate_audio_xh.py` runs the local Simba-TTS model (UBC-NLP,
  CC-BY-4.0; attribution lives in terms.html + the Settings voice line;
  venv `~/.bhasaly-xhosa-tts`; `--faith` writes `audio-xhf/`). Bare letters
  are VITS noise, so its `SPEAK_AS` speaks syllable demos under the letter
  keys. Short isolated words are the model's weak spot — regeneration must
  go through the ASR best-of-N loop (Aug 2026 audit, method in HISTORY.md;
  short-segment ASR CER 0.3–0.7 is artifact, only trust CER on ≥12 chars).
  Match-pair `pairs[i][0]` is SPOKEN — never put grammar notation there.
